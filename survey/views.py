...
def index(request):
    if request.user.is_authenticated:
        ...
        logger.info("Now {} is logged in ".format(request.user.username))
...
def question_list(request, survey_id):
    ...
    logger.info("This is the survey question view and current user is {}".format(m))
    ...
    logger.info("ans data : {}".format(ans_data))
    ...
    if request.method == "POST":
        ...
        try:
            if Employee.objects.get(emp_username=username, emp_password=password):
                ...
                logger.info("{} is just logged in into the employee dashboard :".format(username))
...
def logout(request):
    try:
        logger.info("{} trying to log out from employee dashboard ".format(request.session['username']))
...
def save(request, survey_id):
    m = request.session['username']
    emp = Employee.objects.get(emp_username=m)
    logger.info("{} is saving question answers into the system ".format(m))
    for name in request.POST:
        ...
        if name != "csrfmiddlewaretoken" and name != 'btn_response':
                ...
                if request.POST[name]:
                    if request.POST.getlist(name):
                        ...
                        if request.POST["btn_response"] == "Finish":
                            ...
                            logger.info("Result data : {}".format(survey_result_obj))
            ...
            try:
                ...
                email.send()
                logger.info("Email has been sent to : ", m)
            except SMTPAuthenticationError as e:
                logger.exception("Email error : ", e)
        ...
        for employee_id in request.POST.getlist('emp_id'):
            ...
            if not survey_employee:
                ...
                try:
                    ...
                    email.send()
                    logger.info("Email has been send to ", to_email)
