...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        LOGGER.info("Now %s is logged in  ", request.user.username)
        ...
        if 'username' in request.session:
            ...
            for survey in survey_emp_data:
                survey_feedback = SurveyFeedback.objects.filter(
                    employee_id=emp.id, survey_id=survey.survey_id).count()
                if survey_feedback:
                    if SurveyFeedback.objects.filter(
                            survey_id=survey.survey_id, employee_id=emp.id, flag=True):
            ...
            if not is_record:
                if request.POST[name]:
                    if request.POST.getlist(name):
                        ...
                        survey_status = SurveyEmployee.objects.get(
                            employee=Employee.objects.get(id=emp.id),
